...
(() => {
  ...
  const detintRGB = ([r,g,b]) => [clamp8(invDetint(r)), clamp8(invDetint(g)), clamp8(invDetint(b))];
  const tintRGB   = ([r,g,b]) => [clamp8(applyTint(r)), clamp8(applyTint(g)), clamp8(applyTint(b))];

  // Precompute a tinted version of the palette for robust matching while hovered
  const PALETTE_FOR_MATCH = colorpalette.filter(c => c.name !== 'Transparent') // avoid the duplicate black
  ...
  const maxDelta = (a,b) => Math.max(Math.abs(a[0]-b[0]), Math.abs(a[1]-b[1]), Math.abs(a[2]-b[2]));
    ...
    const code = () => {
      ...
      HTMLCanvasElement.prototype.getContext = function(type, attrs) {
        if (type === 'webgl' || type === 'webgl2' || type === 'experimental-webgl') {
          const patched = Object.assign({ preserveDrawingBuffer: true }, attrs || {});
  ...
  (async () => {
    ...
    if (!gl) { console.warn('[wplace] No WebGL context available; cannot sample.'); return; }

    // HUD
    const hud = document.createElement('div');
    Object.assign(hud.style, {
      position: 'fixed', right: '20px', top: '60%', transform: 'translateY(-50%)', zIndex: 999999,
      font: '16px/1.4 monospace', padding: '12px 16px',
      background: 'rgba(0,0,0,.8)', color: '#fff', borderRadius: '12px',
      pointerEvents: 'none', whiteSpace: 'pre', display: 'flex', flexDirection: 'column', gap: '4px'
    });
    ...
    const sample = () => {
      ...
      const x = Math.max(0, Math.min(dbw - 1, Math.floor(cssX * dbw / rect.width)));
      const y = Math.max(0, Math.min(dbh - 1, Math.floor((rect.height - cssY) * dbh / rect.height)));
      ...
      // De-tint estimate of the original color
      const det = detintRGB([r,g,b]);

      // Try match against tinted palette first (since we’re hovered)
      let match = matchTinted([r,g,b]);
      if (!match) {
        // Optional fallback: if for any reason not tinted, try raw palette
        match = matchRaw([r,g,b]);
      }
      ...
      hud.innerHTML =
        ...
        // `detint≈: ${det[0]},${det[1]},${det[2]}\n` +
        `<div style="display: flex; align-items: center;">${paletteLine}${colorSquare}</div>`;

      // Example: forward event if you need it elsewhere
      // window.postMessage({ type:'wplace:pixel',
      //   x, y, r, g, b, a, hex, detinted: det, palette: match ? { id: match.id, name: match.name } : null
      // }, '*');
    };
